...
async function ensurePhpPool(version) {
  ...
  try {
    if (isWindows()) {
      ...
      // Through a generated launcher, not directly, because php-cgi reads its
      // request limit from the ENVIRONMENT and from nowhere else - and
      // `shell_bg_spawn_direct` passes no environment at all. See
      // `poolLauncher` for what that limit does when it is left alone.
      //
      // Spawning a `.cmd` means spawning cmd.exe, which is safe here
      // specifically: on Windows the supervisor holds a kill-on-close job
      // object, so killing the handle kills the whole tree and php-cgi goes
      // with it (`background.rs`). Its liveness travels with the worker too -
      // cmd.exe waits on the child, so a dead worker is a dead handle.
      const dir = paths.conf("php", version);
      await mkdirp(dir);
      const launcher = join(dir, "pool.cmd");
      await writeText(launcher, poolLauncher(exe, port));
      const handle = await spawn("cmd.exe", ["/c", launcher]);
...
/**
 * The batch file that starts one Windows FastCGI worker.
 *
 * It exists for a single line: `PHP_FCGI_MAX_REQUESTS=0`. The FastCGI SAPI
 * recycles its worker after 500 requests by default, on the assumption that a
 * process manager is watching and will start a replacement. Nothing here is
 * watching, so the worker simply exited and every site on that PHP version
 * answered 502 until somebody restarted something by hand. Measured before the
 * fix, at three different concurrencies: the worker died after 498, 499 and 499
 * requests - the same number every time, which is what gave the cause away. A
 * page load with its assets is a dozen requests, so 500 is an afternoon.
 *
 * 0 means never recycle. That is the right value only because a leak in a local
 * development worker is visible and survivable, and a silent 502 is not.
 *
 * Exported so the self-check can assert on the rendered file rather than on the
 * source text of this module.
 *
 * @param {string} exe Absolute path to `php-cgi.exe`.
 * @param {number} port
 * @returns {string}
 */
export function poolLauncher(exe, port) {
  return [
    "@echo off",
    "rem Generated by the TEDI Dev Environment extension.",
    "rem Edits are lost on the next start.",
    'set "PHP_FCGI_MAX_REQUESTS=0"',
    `"${exe}" -b 127.0.0.1:${port}`,
    "",
  ].join("\r\n");
}
